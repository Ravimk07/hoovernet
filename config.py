...
class Config(object):
    ...
    def __init__(self):
        ...
        act_shape = [512, 512] # patch shape used as input to network - central crop performed after augmentation
        out_shape = [256, 256] # patch shape at output of network
        # aug_shape = [1024, 1024]  # patch shape used during augmentation (larger patch may have less border artefacts)
        # act_shape = [270, 270] # patch shape used as input to network - central crop performed after augmentation
        # out_shape = [80, 80] #

        # if model_mode == "original":
        #     if act_shape != [512, 512] or out_shape != [128, 128]:
        ...
        self.log_dir = "just_a_dummy_name" # where checkpoints will be saved

        # paths to training and validation patches
        self.train_dir_list = [
            "/mnt/imgproc/PROJECTS/PD-L1/data/TPS_score/train_val/consep/train/540x540_300x300" #"train_patches_path"
        ]
        self.valid_dir_list = [
            "/mnt/imgproc/PROJECTS/PD-L1/data/TPS_score/train_val/consep/valid/540x540_300x300"#"valid_patches_path"
        ]
